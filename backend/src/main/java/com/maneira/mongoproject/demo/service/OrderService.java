...
import com.maneira.mongoproject.demo.domain.*;
...
public class OrderService {
    ...
    public Order insert(Order order) {
        ...
        System.out.println("teste teste teste funciona");
        return savedOrder;
    }

    public void delete(String id) {
        Order order = findById(id);
        System.out.println("checking remove process...");
        Date datePayment = order.getDatePayment();
        if (datePayment != null) {
            Client client = clientService.findById(order.getClient().getId());
            System.out.println("executing remove process...");
            ...
            Revenue revenue = revenueService.findByYearAndMonth(year, month);
            if (revenue != null) {
                revenue.setAmount(revenue.getAmount() - order.getTotal());
    ...
    public List<Order> fullSearch(String text, Date minDate, Date maxDate, Double minTotal, Double maxTotal, Integer orderStatus, String client, String product, Sort sort) {
        return repo.fullSearch(text, minDate, maxDate, minTotal, maxTotal, orderStatus, client, product, sort);
    }

    public List<Order> fullSearchWithoutStatus(String text, Date minDate, Date maxDate, Double minTotal, Double maxTotal, String client, String product, Sort sort) {
        return repo.fullSearchWithoutStatus(text, minDate, maxDate, minTotal, maxTotal, client, product, sort);
    }
    ...
    public Order updateOrderStatus(String orderId, OrderStatus newStatus, Date date) {
        ...
        switch (newStatus) {
            ...
            case PAGO:
                ...
                revenueService.updateRevenueForPayment(order.getDatePayment(), order);
                break;
            case CANCELADO:
                setCancel(order);
            default:
