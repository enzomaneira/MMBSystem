...
public interface OrderRepository extends MongoRepository<Order, String> {
    ...
    @Query("{ $and: [ " +
            ...
            "{ orderStatus: ?5 }, " +
            "{ $and: [ " +
            "{ 'client.name': { $regex: ?6, $options: 'i' } }, " +
            "{ 'items.product.name': { $regex: ?7, $options: 'i' } }" +
            "] }" +
            "] }")
    List<Order> fullSearch(String text, Date minDate, Date maxDate, Double minTotal, Double maxTotal, Integer orderStatus, String client, String product, Sort sort);
    ...
    @Query("{ $and: [ " +
            ...
            "] }" +
            "] }")
    List<Order> fullSearchWithoutStatus(String text, Date minDate, Date maxDate, Double minTotal, Double maxTotal, String client, String product, Sort sort);
