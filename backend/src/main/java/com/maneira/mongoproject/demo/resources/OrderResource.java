...
public class OrderResource {
    ...
    public ResponseEntity<List<Order>> fullSearch(
        ...
        if (orderStatusCode != null && !orderStatusCode.isEmpty()) {
            Integer status = URL.convertInteger(orderStatusCode, null);
            result = orderService.fullSearch(text, min, max, minTotal, maxTotal, status, client, product, sort);
        } else {
            result = orderService.fullSearchWithoutStatus(text, min, max, minTotal, maxTotal, client, product, sort);
        }
