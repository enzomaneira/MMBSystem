...
const EditarCliente = () => {
  ...
  const handleSearch = async (event) => {
    ...
    try {
      const response = await fetch(
        `http://52.2.29.147:8080/clients/findByNumber?number=${searchNumber}`
      );
      if (!response.ok) {
  ...
  const handleUpdate = async (event) => {
    event.preventDefault();
    const updateUrl = `http://52.2.29.147:8080/clients/${clientInfo.id}`;
  ...
  const handleDelete = async () => {
    try {
      await fetch(`http://52.2.29.147:8080/clients/${clientInfo.id}`, {
        method: "DELETE",
      });
