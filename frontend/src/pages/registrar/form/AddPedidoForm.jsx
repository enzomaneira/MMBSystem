...
const AddPedidoForm = () => {
  ...
  const createOrderItem = async (orderItemData) => {
    try {
      const response = await fetch("http://52.2.29.147:8080/orderItems", {
        method: "POST",
        headers: {
  ...
  const handleRemoveProduto = (index) => {
    const newProdutos = [...formData.produtos];
    if (newProdutos.length > 1) { // Verifica se há mais de um produto antes de remover
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const response = await fetch("http://52.2.29.147:8080/orders", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
  ...
  return (
    ...
    >
      ...
      <div className={styles.column}>
        ...
        <button
          type="button"
          onClick={() => handleRemoveProduto(formData.produtos.length - 1)}
