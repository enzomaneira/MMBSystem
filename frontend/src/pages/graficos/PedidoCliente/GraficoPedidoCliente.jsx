...
function GraficoPedidoCliente() {
  ...
  const fetchClientData = async () => {
    try {
      const response = await axios.get('http://52.2.29.147:8080/clients');
  ...
  const fetchSalesData = async () => {
    try {
      const response = await axios.get(`http://52.2.29.147:8080/orders/fullSearch?client=${selectedClient}`);
